...
class TweetsSave extends StatefulWidget {
  final String userId;

  const TweetsSave({required this.userId});

  @override
  State<TweetsSave> createState() => _TweetsSaveState();
}

class _TweetsSaveState extends State<TweetsSave> with SingleTickerProviderStateMixin {
  ...
  void _startChatWithUser(UserModel peerUser) async {
    ...
    final conversationId = await ChatService().createOrGetChat(currentUserId, peerUser.uid);
                        ...
                        child: Column(
                          children: [
                            ProfileCardWidget(
                              ...
                              onFollowPressed:
                              isFollowing ? () => _unfollowUser(user) : () => _followUser(user),
                            ...
                            StreamBuilder<QuerySnapshot>(
                              stream: FirebaseFirestore.instance
                                  ...
                                  .where('savedBy', arrayContains: widget.userId)
                                  .snapshots(),
                              builder: (context, snapshot) {
                                if (snapshot.connectionState == ConnectionState.waiting) {
                                  return const Center(child: CircularProgressIndicator());
                                ...
                                if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                                  return const Center(child: Text('لا توجد تغريدات محفوظة.'));
                                }

                                final tweets = snapshot.data!.docs
                                    .map((d) =>
                                    TweetModel.fromMap(d.data() as Map<String, dynamic>, id: d.id))
                                    .where((t) {
                                  final text = t.text.toLowerCase();
                                  final q = searchQuery.toLowerCase();
                                  return q.isEmpty || text.contains(q) || text.contains('#$q');
                                }).toList();
                                ...
                                return ListView.builder(
                                  ...
                                  itemBuilder: (c, i) => TweetTile(tweet: tweets[i]),
                                );
                              },
                            ),
                            const SizedBox(height: 15),
            ...
            child: Directionality(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  const Divider(),
                  _HoverDrawerItem(label: 'تعرف على شطاره', route: '/main'),
...
/// 🔹 هيدر متجاوب: صورة بالأعلى + بنر على الجوال فقط
class _TopHero extends StatelessWidget {
  final bool isMobile;
  const _TopHero({required this.isMobile});

  @override
  Widget build(BuildContext context) {
    if (isMobile) {
      // الجوال: الصورة بالأعلى + بنر أحمر
      return Padding(
      ...
      // الكمبيوتر: الصورة في المنتصف والأعلى مع حد أقصى للعرض
      return Padding(
