...
export async function GET(): Promise<NextResponse<ApiProxyResponse<number>>> {
  try {
    const response = await fetch(CHESS_API_ENDPOINTS.online, {
      method: 'GET',
      headers: {
    ...
    const count = normalizeCountResponse(raw, ['online', 'count', 'total']);

    return NextResponse.json({ success: true, data: count }, { status: 200 });
  } catch (error) {
    console.error('[chess/online] proxy error:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to fetch online users count' },
      { status: 500 },
    );
  }
}
